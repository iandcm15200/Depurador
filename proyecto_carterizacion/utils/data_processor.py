...
def depurar_datos(df: pd.DataFrame, hours: int = 24, days: int = None) -> pd.DataFrame:
    """
    Depura el DataFrame:
    - Mantiene solo columnas necesarias si están presentes.
    - Convierte PaidDate a datetime (dayfirst True para dd/mm/yyyy).
    - Filtra por últimas 'hours' horas o últimos 'days' días si se provee days.
    ...
    try:
        ...
        if 'PaidDate' in df.columns:
            ...
            if hours is not None:
                fecha_limite = now - pd.Timedelta(hours=hours)
                df = df[df['PaidDate'] >= fecha_limite]
            elif days is not None:
                fecha_limite = now - pd.Timedelta(days=days)
                df = df[df['PaidDate'] >= fecha_limite]
        else:
