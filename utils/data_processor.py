...
def depurar_datos(df: pd.DataFrame, hours: int = 24, days: int = None, timestamp_referencia: datetime = None) -> pd.DataFrame:
    ...
    - Filtra registros que estén dentro de las últimas `hours` horas (por defecto 24)
    ...
    try:
        ...
        # Si muchos nulos, intentar con segundos o sin hora y dayfirst=True
        ...
        # Si aún hay muchos nulos, intentar extracciones básicas (solo fecha parte)
        ...
        # Aplicar filtro temporal: últimas `hours` horas (por defecto 24) hasta timestamp_referencia
        antes = len(df)
        ...
        # Formatear PaidDate como texto DD/MM/YYYY HH:MM para salida (antes del filtrado se mantuvo datetime)
